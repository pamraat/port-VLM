function [a,b,a1,b1,a2,b2,a3,b3,a4,b4,nor]=infcoeffblk1(x,y,z,aoa)
n=size(x,1)-1;
m=(size(x,2)-1)/2;
for k=1:5
for i=1:n
...
for j=1:2*m 
    ...
    for l=1:2*m
        ...
        elseif(g1==3)
        a2(2*m*(i-1)+j,2*m*(k-1)+l)=-dot(v,nor(i,:,j,1));
        b2(2*m*(i-1)+j,2*m*(k-1)+l)=-dot(vw,nor(i,:,j,1));
        elseif(g1==4)
        a3(2*m*(i-1)+j,2*m*(k-1)+l)=-dot(v,nor(i,:,j,1));
        b3(2*m*(i-1)+j,2*m*(k-1)+l)=-dot(vw,nor(i,:,j,1));
        elseif(g1==5)
        a4(2*m*(i-1)+j,2*m*(k-1)+l)=-dot(v,nor(i,:,j,1));
        b4(2*m*(i-1)+j,2*m*(k-1)+l)=-dot(vw,nor(i,:,j,1));
        end
    end
    end
    end
