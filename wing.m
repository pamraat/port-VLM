...
for i=1:n
for j=1:2*m
    rhs(2*m*(i-1)+j)=-dot(vinf,nor(i,:,j));
end
end
rhs=transpose(rhs);
...
for j=1:2*m
    ...
    ls(i,j)=rho*norm(vinf)*gama(2*m*(i-1)+j)*(y(i,j+1)-y(i,j));
    cls(i,j)=ls(i,j)/(0.5*rho*norm(vinf)*wingar);
    else
    ...
    ls(i,j)=rho*norm(vinf)*(gama(2*m*(i-1)+j)-gama(2*m*(i-2)+j))*(y(i,j+1)-y(i,j));
    cls(i,j)=ls(i,j)/(0.5*rho*norm(vinf)*wingar);
    end
end
end
...
fprintf('Lift is %f\n', L);
cl=L/(0.5*rho*norm(vinf)*wingar);
fprintf('Cl is %f\n', cl);

plot(cls(1,m+1:2*m)/cl);
